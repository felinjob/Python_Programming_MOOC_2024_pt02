...
def most_goals(bp: list) -> str:
    goals = max(bp, key=lambda goals: goals.goals)    
    return goals.name
def most_points(bp: list) -> tuple:
    points = max(bp, key=lambda x: x.number + x.passes)
    return (points.name, points.number)

def least_minutes(bp: list) -> str:
    return min(bp, key=lambda minutes: minutes.minutes)
